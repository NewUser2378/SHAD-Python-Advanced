...
class BannerStat:
    ...
    def compute_ctr(self, default_ctr: float) -> float:
        ...
        if self.shows == 0:
            ...
            return self.clicks / self.shows  # деление клик на показ
...
class BannerStorage:
    ...
    def add_click(self, banner_id: str) -> None:
        if banner_id in self._banner_dict:  # проверка баннера.
...
class EpsilonGreedyBannerEngine:
    ...
    def __init__(self, banner_storage: BannerStorage, random_banner_probability: float):
        ...
        if banner_storage.is_empty():
            raise EmptyBannerStorageError("Storage is empty!")  # проверка на пустой storage
    ...
    def show_banner(self) -> str:
        ...
        """
        if random.random() <= self._epsilon:  # вероятность
    ...
    def send_click(self, banner_id: str) -> None:
        ...
        """
        try:
            self._storage.add_click(banner_id)  # было add_show на add_click.
