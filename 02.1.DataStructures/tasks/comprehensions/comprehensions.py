...
def get_unique_page_ids(records: list[tp.Mapping[str, tp.Any]]) -> set[int]:
    ...
    """
    return {record['PageID'] for record in records if 'PageID' in record}
...
def get_unique_page_ids_visited_after_ts(
    ...
    return {
        ...
        if 'PageID' in record and 'EventTime' in record and record['EventTime'] > ts
    }
...
def get_unique_user_ids_visited_page_after_ts(
    ...
    return {
        record['UserID']
        for record in records
        if 'EventTime' in record and record['EventTime'] > ts and record['PageID'] == page_id
    }
...
def get_events_by_device_type(
    ...
    return [
        record
        for record in records
        if 'DeviceType' in record and record['DeviceType'] == device_type
    ]
...
def get_region_ids_with_none_replaces_by_default(
    ...
    """
    Extract visited regions with order preservation.
    If region not defined, replace it by default region id
    :param records: records of hit-log
    :return: region ids
    """
    return [
        record['RegionID']
        if 'RegionID' in record and record['RegionID'] is not None
